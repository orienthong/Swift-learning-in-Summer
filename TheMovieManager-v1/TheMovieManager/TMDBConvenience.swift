...
extension TMDBClient {
    ...
    */
    ...
    private func getSessionID(requestToken: String?, completionHandlerForSession: (success: Bool, sessionID: String?, errorString: String?) -> Void) {
        ...
        taskForGETMethod(Methods.AuthenticationSessionNew, parameters: parameters) {
            (results, error) in
            if let error = error {
                print(error)
                completionHandlerForSession(success: false, sessionID: nil, errorString: "Login Failed, disable to getSessionID")
            } else {
                if let sessionID = results["session_id"] as? String {
                    completionHandlerForSession(success: true, sessionID: sessionID, errorString: nil)
                } else {
                    print("Could not find \(TMDBClient.JSONResponseKeys.SessionID) in \(results)")
                    completionHandlerForSession(success: false, sessionID: nil, errorString: "Login failed with getSessionID")
                }
            }
    ...
    private func getUserID(completionHandlerForUserID: (success: Bool, userID: Int?, errorString: String?) -> Void) {
        ...
        taskForGETMethod(Methods.Account, parameters: parameters) {
            (results, error) in
            if let error = error {
                print(error)
                ...
                if let userID = results[TMDBClient.JSONResponseKeys.UserID] as? Int {
                    ...
                    print("Could not find \(TMDBClient.JSONResponseKeys.UserID)")
                    completionHandlerForUserID(success: false, userID: nil, errorString: "Login Failed (User ID)")
                }
            }
        }
        ...
        taskForGETMethod(mutableMethod, parameters: parameters) {
            ...
            if let error = error {
                ...
                if let results = results[TMDBClient.JSONResponseKeys.MovieResults] as? [[String: AnyObject]] {
                    ...
                    completionHandlerForWatchlist(result: nil, error: NSError(domain: "getWatchlistMovies", code: 0, userInfo: [NSLocalizedDescriptionKey: "Could not parse getWatchlistMovies"]))
                }
            }
        }
