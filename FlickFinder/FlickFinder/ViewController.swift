...
class ViewController: UIViewController {
    ...
    private func bboxString() -> String {
        if let latitude = Double(latitudeTextField.text!), let longtitude = Double(longitudeTextField.text!) {
            let minimumLon = max(longtitude - Constants.Flickr.SearchBBoxHalfWidth,Constants.Flickr.SearchLonRange.0)
    ...
    private func displayImageFromFlickrBySearch(methodParameters: [String:AnyObject]) {
        ...
        let task = session.dataTaskWithRequest(request) { (data, response, error) in
            ...
            if photosArrary.count == 0 {
                ...
                let randomPhotoIndex = Int(arc4random_uniform(UInt32(photosArrary.count)))
